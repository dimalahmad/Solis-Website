...
import { Box, CssBaseline, Drawer, AppBar, Toolbar, Typography, IconButton, List, ListItem, ListItemIcon, ListItemText, Avatar, Badge, InputBase, Switch, Paper, ListItemButton } from '@mui/material';
import { Dashboard, ListAlt, Assessment, Settings, Notifications, Search, Person, Brightness4, Brightness7 } from '@mui/icons-material';
...
import './App.css';
import logo from './assets/react.svg'; // Ganti dengan logo baru jika ada
    ...
    <ThemeProvider theme={theme}>
      ...
      <Box sx={{ display: 'flex', minHeight: '100vh', background: theme.palette.background.default }}>
        ...
        <Box component="main" sx={{ flexGrow: 1, ml: 0 }}>
          {/* Topbar */}
          <AppBar position="static" elevation={0} sx={{ background: darkMode ? 'rgba(16,22,36,0.95)' : '#fff', color: '#1E3A8A', borderBottom: '1px solid #E5E7EB' }}>
            <Toolbar>
              <Box sx={{ flexGrow: 1, display: 'flex', alignItems: 'center' }}>
                <InputBase
                  placeholder="Cari laporan, pengguna..."
                  sx={{ ml: 1, flex: 1, background: darkMode ? '#232946' : '#F1F5F9', px: 2, py: 0.5, borderRadius: 2, fontSize: 16, width: 320, color: darkMode ? '#fff' : '#1E3A8A' }}
          ...
          {/* Area Konten */}
          <Box sx={{ p: 3, width: 'calc(100vw - 240px)', mx: 0 }}>
            <Routes>
        ...
        {/* Floating Theme Switcher Dongle */}
        <Paper elevation={6} sx={{ position: 'fixed', bottom: 32, right: 32, zIndex: 2000, p: 1.5, borderRadius: 8, display: 'flex', alignItems: 'center', background: darkMode ? '#232946' : '#fff', boxShadow: '0 4px 24px #0002' }}>
