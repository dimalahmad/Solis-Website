...
const DetailLaporan = ({ darkMode = true }) => {
  ...
  const kategoriList = [
    { label: 'Kebakaran', icon: <FireIcon sx={{ color: '#ef4444' }} /> },
    { label: 'Kriminal', icon: <PoliceIcon sx={{ color: '#3b82f6' }} /> },
    { label: 'Medis', icon: <MedicalIcon sx={{ color: '#22c55e' }} /> },
    { label: 'Lainnya', icon: <CategoryIcon sx={{ color: '#64748b' }} /> },
  ];
  ...
  const handleKategoriChange = (event) => {
    setLaporan(prev => ({ ...prev, kategori: event.target.value }));
  };
      ...
      <Box sx={{
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, flexWrap: 'wrap' }}>
          ...
          <Box sx={{
            ...
            alignItems: 'center',
            gap: 1,
            bgcolor: darkMode ? '#1a2236' : '#e2e8f0',
            px: 2,
            py: 1,
            borderRadius: 2,
          }}>
            <TimeIcon sx={{ color: darkMode ? '#1E90FF' : '#1E3A8A' }} />
            <Typography fontWeight="bold" fontSize={18} sx={{ color: darkMode ? '#fff' : '#1E3A8A' }}>
              {laporan.durasi}
            </Typography>
          </Box>

          {/* Status Selector dengan background */}
          <FormControl size="small" sx={{
            minWidth: 150,
            backgroundColor: statusColors[laporan.status],
            borderRadius: 2,
          }}>
            <InputLabel
              id="status-label"
              sx={{ color: darkMode ? '#1E90FF' : '#1E3A8A' }}
            >
              Status
            </InputLabel>
            <Select
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
              ...
              <Box>
                ...
                <Grid container spacing={2}>
                  ...
                  <Grid item xs={12} sm={6}>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 2 }}>
                      <TimeIcon sx={{ color: darkMode ? '#ef4444' : '#dc2626' }} />
                      <Box>
                        <Typography variant="caption" sx={{ opacity: 0.8 }}>Waktu</Typography>
                        <Typography>{laporan.waktu}</Typography>
                      </Box>
                    </Box>
                  </Grid>
                ...
                <FormControl fullWidth size="small" sx={{ mb: 2 }}>
                  <Select
                    ...
                    renderValue={(selected) => {
                      const selectedKategori = kategoriList.find(k => k.label === selected);
                      return (
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                  ...
                  >
                    {kategoriList.map((kategori) => (
                      <MenuItem key={kategori.label} value={kategori.label}>
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                          {kategori.icon}
                          <Typography>{kategori.label}</Typography>
                        </Box>
                      </MenuItem>
                    ))}
                  </Select>
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3, height: '100%' }}>
            ...
            <Paper elevation={3} sx={{
              ...
              <Box sx={{
                ...
                <Box>
                  <Typography variant="subtitle1" fontWeight="bold">{laporan.nama}</Typography>
              ...
              <Box sx={{
                ...
                {laporan.chatList.map((chat, idx) => (
                  ...
                  >
                    <Typography sx={{ fontSize: 14 }}>{chat.text}</Typography>
                    <Typography
                      sx={{
                        fontSize: 10,
                        position: 'absolute',
